...
\item However, for them to be ‘superpositions’, there is a constraint
  ...
  themselves.
% \footnote{The constraint opaquely relates quantum
...
\item The full linear space will comprise ordered pairs of the
  form~$(\alpha_1\eblack+\alpha_2\ewhite,\beta_1\eblack+\beta_2\ewhite)$,
  where~$|\alpha_1|^2+|\alpha_2|^2=|\beta_1|^2+|\beta_2|^2=|\alpha_1|^2+|\beta_1|^2=|\alpha_2|^2+|\beta_2|^2=1$.\footnote{Hint:
    the squares are {\it probabilities}.}
...
\item Now let us make it clear that the state space represents {\it
  ...
  be the
  % superposition~$\ebew{e^{i\pi/4}}{e^{i\pi/4}}+\eweb{e^{i\pi/4}}{e^{i\pi/4}}$. We
  % might have used~$\sqrt{1/2}$ instead of~$e^{i\pi/4}$, but complex
  % number multipliers let us more easily represent what a Stern-Gerlach
  % Shutter does.
...
\item Actually, hardly anyone one knows what a Stern-Gerlach Shutter
  ...
  by~$e^{-i\phi/2}$.\footnote{One can deduce these operations from
    intermediate results of the earlier solution. Now deduce the other
    way: a Stern-Gerlach magnet is a kind of shutter or valve, whose
    effective areas are squares of sinusoids. Therefore problems
    involving Stern-Gerlach magnets can be solved without quantum
    mechanics.}

% \item Thus the new state of the calculation, taking into account the
%   shutters, is the superposition
%   \startformula
%     e^{i3\pi/4}\ebew
%   \stopformula
