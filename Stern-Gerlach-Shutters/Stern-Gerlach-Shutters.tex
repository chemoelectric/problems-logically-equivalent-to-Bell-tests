...
\define\eblack{{e^{\rm b}}}
\define\ewhite{{e^{\rm w}}}
...
\item Either of the two pellets has its own smaller linear
  space. Let~$\eblack$ represent the basis vector for a black pellet
  and~$\ewhite$ the basis vector for a white pellet. Then these
  vectors form a linear space. So-called ‘superpositions’ can be
  formed from linear combinations of the two basis vectors.
...
\item The full linear space will have as basis ordered pairs of the
  form~$(\eblack,\ewhite)$ or~$(\ewhite,\eblack)$. It also is subject
  to the ‘superposition’ constraint.
