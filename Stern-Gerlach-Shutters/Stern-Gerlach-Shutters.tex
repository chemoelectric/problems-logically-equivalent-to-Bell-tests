...
\item Refer to the setting gauge’s setting as~$\phi$. The projection
...
\item Let us solve for the case~$\phi_1=\phi_{01}\in[0,\pi]$,
  ...
  were no shutter at all, and in fact one need not bother shooting
  at~Shutter~2.
...
\item Suppose one is given~$\phi_1$ and~$\phi_2$ as the respective
  settings. Assume~$\phi_1\ge\phi_2$. (Otherwise reverse the roles.)
  Then let~$\phi_{01}=\phi_1-\phi_2$ and~$\phi_{02}=\phi_2-\phi_2=0$,
